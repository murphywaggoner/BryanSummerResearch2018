...
DSSurvey %>% 
  ...
  # summarize(n = n()) %>% 
  filter(!is.na(clinicdistance)) %>%
  ggplot(aes(x = clinicdistance, y = DSvisit, color = income)) +
  geom_point() +   
  geom_jitter() +
  # geom_smooth(aes(x = as.numeric(clinicdistance), y = as.numeric(DSvisit), weight = income)) +
  ggtitle('Distance from Clinic vs Frequency of Visits') 
...
DSSurvey %>% 
  filter(!is.na(DSeducation)) %>% 
  ggplot(aes(x = DSvisit, y = DSeducation, color =  DSage)) +
  geom_point(na.rm = TRUE, size = 10) +
